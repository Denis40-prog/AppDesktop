  ...
  constructor(server: Server, options: ConnectionPoolOptions) {
    ...
    if (this.options.minPoolSize > this.options.maxPoolSize) {
      throw new MongoInvalidArgumentError(
        'com.ProjetSport.Connection pool minimum size must not be greater than maximum pool size'
      );
    }
    ...
    if (!credentials) {
      return callback(
        new MongoMissingCredentialsError(
          'com.ProjetSport.Connection is missing credentials when asked to reauthenticate'
        )
      );
    }
  ...
  private createConnection(callback: Callback<Connection>) {
    ...
    connect(connectOptions, (err, connection) => {
      if (err || !connection) {
        ...
        callback(err ?? new MongoRuntimeError('com.ProjetSport.Connection creation failed without error'));
        return;
      }
